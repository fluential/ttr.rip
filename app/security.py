...
def is_valid_auth_key(key: str) -> bool:
    """
    Validates that the auth key matches the format generated by generate_auth_key():
    a 32-character URL-safe base64 string ([A-Za-z0-9_-]{32}).
    """
    try:
        return bool(key and re.fullmatch(r"[A-Za-z0-9_-]{32}", key))
    except Exception:
        return False
...
async def get_public_user_from_key(
    ...
    if not auth_key or not is_valid_auth_key(auth_key):
